...
// Get all blood bank facilities for debugging/setup
$allFacilities = $pdo->query("SELECT * FROM facilities WHERE type = 'blood_bank' ORDER BY name")->fetchAll(PDO::FETCH_ASSOC);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    if ($_POST['action'] === 'assign_existing') {
        ...
        if (!$name || !$address || !$city || !$phone || !$email) {
            ...
            try {
                $insert = $pdo->prepare("INSERT INTO facilities (name, type, address, city, phone, email, admin_id) VALUES (?, 'blood_bank', ?, ?, ?, ?, ?)");
...
<head>
    ...
    <title>Facility Setup - Blood Bank Portal</title>
    ...
    <style>
        ...
        .status-card {
            background: linear-gradient(135deg, var(--bloodbank-purple) 0%, rgba(156, 39, 176, 0.8) 100%);
            color: white;
            padding: 2rem;
            border-radius: 12px;
            margin-bottom: 2rem;
...
<body>
    ...
    <div class="dashboard-container">
        <div class="facility-setup">
            ...
            <p style="color: #ffffff;">Connect your blood bank admin account to a facility</p>
            ...
            <div class="status-card">
                ...
                <?php if ($facility): ?>
                    ...
                    <p style="margin-top: 1rem; font-size: 0.9em;">
                        You can now manage inventory in <a href="inventory.php" style="color: inherit; text-decoration: underline;">Inventory</a>
                    </p>
                <?php else: ?>
                    <div class="facility-info">
                ...
                <div class="form-section">
                    ...
                    <form method="POST">
                        <div class="form-group">
                            <label for="facility_id">Select Blood Bank</label>
                            <select id="facility_id" name="facility_id" required>
                                <option value="">-- Choose a blood bank --</option>
                                <?php foreach ($allFacilities as $fac): ?>
                                    <option value="<?php echo $fac['id']; ?>">
                                        <?php echo htmlspecialchars($fac['name'] . ' (' . $fac['city'] . ')'); ?>
                                    </option>
                                <?php endforeach; ?>
                            </select>
                        </div>
                        <input type="hidden" name="action" value="assign_existing">
                        <button type="submit" class="btn btn-primary" style="background: var(--bloodbank-purple);">Assign Facility</button>
                    </form>
                </div>
                ...
                <div class="form-section">
                    ...
                    <form method="POST">
                        <div class="form-group">
                            ...
                            <input id="name" name="name" placeholder="e.g., Central Blood Bank" required>
                        </div>
                        <div class="form-group">
                        ...
                        <div class="form-group">
                            ...
                            <input id="email" name="email" type="email" placeholder="contact@bloodbank.com" required>
                        </div>
                        <input type="hidden" name="action" value="create_new">
                        <button type="submit" class="btn btn-primary" style="background: var(--bloodbank-purple);">Create Facility</button>
                    </form>
                </div>
            <?php endif; ?>
            ...
            <div class="form-section" style="background: rgba(142, 68, 173, 0.08); border: 2px solid var(--bloodbank-purple); border-radius: 8px; padding: 1.5rem;">
                ...
                <p style="color: #ffffff; font-weight: 500; margin: 0.75rem 0;"><strong>Total Blood Banks in System:</strong> <span style="color: #ffd700; font-weight: 600;"><?php echo count($allFacilities); ?></span></p>
